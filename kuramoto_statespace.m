...
% - Analitycal solution
% Adt = sparse(expm(A * dt));
% - Crank-Nicolson approximation
Adt = sparse((eye(size(A)) - A * dt/2)\(eye(size(A)) + A * dt/2));
...
Bd = ks_init_input(posd,sigd,xx,zz,LX,LZ);
...
Bu = ks_init_input(posu,sigu,xx,zz,LX,LZ);
...
return
%% Compare time steppers
eigAN = eig(expm(A * dt)); % Analitycal solution
eigCN = eig((eye(size(A)) - A * dt/2)\(eye(size(A)) + A * dt/2)); % Crank-Nicolson

figure(1000); polar(angle(eigAN),abs(eigAN),'or'); hold on;
              polar(angle(eigCN),abs(eigCN),'+b'); hold off; grid on
legend('Analitical solution','Crank-Nicolson','Location','NE')